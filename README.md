...
| 标记 | 含义 |
| --- | --- |
| √ | 该 OS 启用此资源（`status = "okay"` 或未写 status） |
| × | 该 OS 的 DTS 中存在此节点但 `status = "disabled"` |
| - / 空 | 该 OS 的 DTS 中不存在此节点 |
| 共享: OS-A, OS-B | 备注列：多个 OS 同时启用，详见「共享资源清单」 |

**颜色语义**（底纹）：

| 颜色 | 含义 | 出现位置 |
| --- | --- | --- |
| 黄色 | 多 OS 共享：多个 OS 同时启用，需确认是否为预期设计（如共享内存、IPC 区、带宽控制寄存器） | 资源/内存分配矩阵的共享行、外设分配中多 OS 启用行、总览共享统计 |
| 粉红（红字） | 冲突：需处理（外设竞争、地址/中断/CPU 重叠等） | 冲突报告 |
| 绿色 | 无冲突：总览冲突计数为 0、冲突报告「未检测到冲突」 | 总览、冲突报告 |
| 蓝色 | 表头 | 各 Sheet 首行 |
| 浅蓝 | 类别分组行（GIC、GPU、UART 等） | 资源/外设分配矩阵 |

简单记：**黄色 = 共享（提醒确认）、粉红 = 冲突（需要处理）**。
